...
- Local `.eml` parsing for safe headers and visible body text, with RFI question/response and Submittal status evidence boundaries. Paired HTML blockquote boundaries keep quoted history separate without losing current reply text after the quote. Spaced CSI-style Submittal identifiers are normalized from explicit body, subject, or filename metadata. Attachments remain inert until a reviewer explicitly imports one through the normal capacity, hash and deduplication controls; the parent email and selected attachment then appear together in workflow review without inheriting approval or authority. HTML active content and remote resources are not executed or fetched.
