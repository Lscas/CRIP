...
- A reviewer may compare detected and effective workflow metadata, then apply or reset an exact current-run RFI/Submittal/Other correction. The overlay is version-checked and append-only audited; it never rewrites parser summaries, evidence, candidates, source files, or starts a model call.
