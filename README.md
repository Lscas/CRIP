...
- Ask the selected project run a plain-English question. CIRP uses a run-scoped SQLite full-text index to rank bounded evidence—including exact RFI/Submittal identifiers, Email text and source locators—then sends only that context through the configured budgeted model when the user submits. Existing databases are backfilled locally; systems without FTS5 use a complete compatibility scan. Exact source quotations appear directly below the answer, and Mock mode retrieves context but never fabricates an answer.
