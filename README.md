...
- Exact unfiltered questions such as `How many RFIs, Submittals, and Emails are in this run?` reuse the complete cached workflow index and answer before evidence retrieval, with no model request or budget action. RFI/Submittal totals are identifier groups; Email totals are unique analyzed Email, `.eml` or `.msg` files. Qualified counts such as open RFIs or files mentioning a topic stay on the ordinary evidence path.
