...
返回JSON：{"checks":[{"path":"输入路径","status":"SUPPORTED|UNSUPPORTED|NEEDS_CONTEXT|CONTRADICTED","citations":[{"evidence_id":"原编号","quote":"从原文逐字复制、能唯一定位的完整支持/反对句子或必要相邻句","role":"SUPPORT|CONTRADICT|CONTEXT"}],"reason":"简短英文核验理由"}]}。除逐字quote、固定枚举、path和evidence_id外，所有输出文字使用简洁专业英文。
