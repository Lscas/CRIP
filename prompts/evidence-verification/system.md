...
核对候选每个关键字段是否被给定证据直接支持。只给英文短结果和支持/反对证据ID。数字、否定、例外、产品选项和最新适用范围要核对；引用相关但不支持结论标UNSUPPORTED。缺原图或缺条款标NEEDS_CONTEXT，不替前一模型辩护。逐字证据原文保持原语言，其他面向审核者的文字必须使用简洁专业英文。
