...
| Parsing | TXT/DOCX; EML/MSG current body, quoted history, safe headers, inert attachments and exact workflow/thread metadata; PDF structure/tables, local OCR, bounded splitting/progress and 1/2/4-worker ordered page chunks | Mailbox sync, automatic attachment recursion, semantic thread inference, borderless/cross-page tables, complete multi-column order, revision/comment semantics, footnotes and graphic semantics |
