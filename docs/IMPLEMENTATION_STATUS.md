...
| 材料/QA | 原子文本要求到受控候选；同明确Tag属性比较；字段证据；Spec locator可补充CSI Section；发布前确定性排除属性标题/泛称材料名、非执行QA、重复属性、证据范围错挂、跨文档/非相邻属性引用、RFI问句、被拒Submittal和仅存在于邮件引用历史的要求；RFI答复、Submittal与Email来源候选保持条件性和人工审核 | 完整产品选项、实体多视图归并、结构化数量映射、复杂条件和广义语义质量评估；RFI/Submittal审批权限及语义关系裁决 |
