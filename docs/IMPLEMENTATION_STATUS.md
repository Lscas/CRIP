...
| 解析 | TXT行号；DOCX正文/简单表格；RFC风格EML把当前正文与显式引用历史分开，HTML blockquote成对标记使关闭引用块后的可见回复仍保留为当前正文；附件保持不活动直到显式选择，Message-ID关系只保留哈希，多值In-Reply-To逐个精确哈希并进入同一线程；RFI问题/答复和Submittal显式状态locator；只接受含数字的工作流编号，正文/Subject/文件名中的带空格CSI式Submittal编号经同一严格规则规范化，常见明确状态规范化，同一多页来源的不同显式状态保留组合值并标歧义，无角色RFI保持未知；PDF文字词坐标及确定性页面类型；可识别的Spec Section/Part/条款和有边框表格行保留在证据locator；RapidOCR/ONNX本机OCR及bbox/confidence；长文本按字符和UTF-8字节有界拆分；PDF 300秒上限与页面进度保留；多个文档并行，单个PDF按4页有界块使用1/2/4个本地进程且按页序合并 | MSG、邮箱同步、自动附件递归、语义线程推断和非精确关系推断；无边框/跨页表格、多栏完整阅读顺序、批注修订完整解释、脚注及图形语义关系；小PDF的进程启动成本可能抵消并行收益，单个超慢页面仍可能无进度可保留 |
