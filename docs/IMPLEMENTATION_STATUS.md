...
EML prefers plain text, else safe HTML; history/signatures/attachments stay inert. Exact IDs persist; malformed/fuzzy/address forms stay unlinked. Official Response is RFI-only. Known RFI statuses remain metadata; known Submittal responses use existing groups. Conflicts stay ambiguous without precedence/authority inference. Route: Subject, body, filename.
