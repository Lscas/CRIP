...
| 项目/任务 | SQLite WAL、事务、单运行锁、输入快照、暂停/恢复/取消/中断状态；每个运行可选1/2/4个本地文档处理worker并记录阶段耗时、已处理页数和模型等待/响应汇总 | 多租户/生产任务队列、进程级资源隔离；尚无真实吞吐基准 |
| 解析 | TXT行号；DOCX正文/简单表格；PDF文字词坐标；RapidOCR/ONNX本机OCR及bbox/confidence；长文本按字符和UTF-8字节有界拆分；PDF 300秒上限与页面进度保留；多个文档可由最多4个本地worker并行解析 | 单个PDF内部页级并行、批注修订完整解释、跨页/脚注及图形语义关系；单个超慢页面仍可能无进度可保留 |
...
| 模型 | mock端到端；DeepSeek文本非思考、DeepSeek V4 Flash Vision受限整页页面路由及Gemini文本minimal适配；客户可在本机页面配置OpenAI-compatible远端文本API或回环本地模型，端点与模型绑定运行身份；视觉仅选择大幅面、低文字图形或高密度图形页；相同证据范围的字段核验最多4项合并；指定59页图纸与833页规格书的历史合并运行完成2/2文件、222/222视觉页和全部核验 | 通用入口的供应商专用参数、视觉、自动模型发现和非回环LAN；施工准确率、局部高分辨率裁剪和L2升级；本轮选择性视觉策略尚无live质量对比 |
