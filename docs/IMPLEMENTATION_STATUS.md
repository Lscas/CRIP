...
EML的非空纯文本alternative保持优先；纯文本全部为空白时使用非空安全HTML，因此不会因空壳plain部分丢失RFI回复或Submittal状态。Outlook把`From / Sent / To / Subject`等显式引用头压在同一可见行时仍能开始引用历史，旧问题不会混入当前RFI答复角色。父邮件解析遇到附件即停止向下遍历，附加EML正文及其内部文件不会污染父证据、父附件清单或RFI角色。
