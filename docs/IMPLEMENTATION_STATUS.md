...
| 解析 | TXT行号；DOCX正文/简单表格；RFC风格EML安全头信息与可见正文（附件仅登记）；RFI问题/答复和Submittal显式状态locator；PDF文字词坐标及确定性页面类型；可识别的Spec Section/Part/条款和有边框表格行保留在证据locator；RapidOCR/ONNX本机OCR及bbox/confidence；长文本按字符和UTF-8字节有界拆分；PDF 300秒上限与页面进度保留；多个文档并行，单个PDF按4页有界块使用1/2/4个本地进程且按页序合并 | MSG、邮箱同步、附件递归、引用邮件/线程重建；无边框/跨页表格、多栏完整阅读顺序、批注修订完整解释、脚注及图形语义关系；小PDF的进程启动成本可能抵消并行收益，单个超慢页面仍可能无进度可保留 |
...
| 材料/QA | 原子文本要求到受控候选；同明确Tag属性比较；字段证据；Spec locator可补充CSI Section；发布前确定性排除属性标题/泛称材料名、非执行QA、重复属性、证据范围错挂、跨文档/非相邻属性引用、RFI问句和被拒Submittal；RFI答复、Submittal与Email来源候选保持条件性和人工审核 | 完整产品选项、实体多视图归并、结构化数量映射、复杂条件和广义语义质量评估；RFI/Submittal审批权限和跨文件线程关系 |
