...
EML prefers non-empty plain text and falls back to safe HTML only when plain alternatives are blank. Exact history and signature boundaries stay reviewable but cannot route or enter extraction; attachments remain inert. Primary workflow order is Subject, body heading, then filename, and weaker identifiers remain references without donating role or status.
