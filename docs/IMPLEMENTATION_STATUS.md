...
EML: one body/alternative (plain > safe HTML); conflicts warn, no merge. History/signatures/nested `message/*` inert. Exact digit IDs keep distinct same-line markers; duplicates collapse; fuzzy/address forms do not link. Route valid Subject ID > body > filename; invalid Subject stays evidence. Official Response is RFI-only; statuses neutral, conflicts ambiguous.
