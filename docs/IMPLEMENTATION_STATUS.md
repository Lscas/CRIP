...
| 模块 | 当前实际行为 | 未完成 |
|---|---|---|
| 上传 | 网页文件/文件夹；客户配置的Autodesk APS/Procore只读目录浏览和选定文件导入；审核者可检查EML附件元数据并显式导入单个附件，父邮件和附件身份随每次导入保留；统一使用4MiB顺序分片、项目容量门禁、原文件SHA-256和别名去重 | 10GB压力测试、断点垃圾回收、云端对象存储、病毒扫描；EML自动递归/批量附件导入；交互OAuth/刷新/供应商端撤销、递归批量导入、变化轮询和持续同步；真实租户验收 |
