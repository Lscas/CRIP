...
EML: one body/alternative (plain > safe HTML); conflicts warn. Related uses declared root/first child; other resources, history, signatures and nested messages stay inert. Exact IDs persist/dedupe; fuzzy/address forms do not link. Route valid Subject ID > body > filename.
