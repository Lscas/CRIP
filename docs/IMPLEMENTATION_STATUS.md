...
| 模块 | 当前实际行为 | 未完成 |
|---|---|---|
| 上传 | 网页文件/文件夹；客户配置的Autodesk APS/Procore只读目录浏览和选定文件导入；统一使用4MiB顺序分片、项目容量门禁、原文件SHA-256和别名去重 | 10GB压力测试、断点垃圾回收、云端对象存储、病毒扫描；交互OAuth/刷新/供应商端撤销、递归批量导入、变化轮询和持续同步；真实租户验收 |
...
| 解析 | TXT行号；DOCX正文/简单表格；RFC风格EML把当前正文与显式引用历史分开，附件仅登记，Message-ID关系只保留哈希；RFI问题/答复和Submittal显式状态locator；精确编号的RFI/Submittal/Email关系索引；PDF文字词坐标及确定性页面类型；可识别的Spec Section/Part/条款和有边框表格行保留在证据locator；RapidOCR/ONNX本机OCR及bbox/confidence；长文本按字符和UTF-8字节有界拆分；PDF 300秒上限与页面进度保留；多个文档并行，单个PDF按4页有界块使用1/2/4个本地进程且按页序合并 | MSG、邮箱同步、附件递归、语义线程推断和非精确关系推断；无边框/跨页表格、多栏完整阅读顺序、批注修订完整解释、脚注及图形语义关系；小PDF的进程启动成本可能抵消并行收益，单个超慢页面仍可能无进度可保留 |
