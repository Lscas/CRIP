...
| 模块 | 当前实际行为 | 未完成 |
|---|---|---|
| 上传 | 网页文件/文件夹；客户配置的Autodesk APS/Procore只读目录浏览和选定文件导入；审核者可检查EML附件元数据并显式导入单个附件，父邮件和附件身份随每次导入保留并在运行Workflow中并列显示，重复关系合并显示但底层事件不丢失；统一使用4MiB顺序分片、项目容量门禁、原文件SHA-256和别名去重 | 10GB压力测试、断点垃圾回收、云端对象存储、病毒扫描；EML自动递归/批量附件导入；交互OAuth/刷新/供应商端撤销、递归批量导入、变化轮询和持续同步；真实租户验收 |
...
| Parsing | TXT line locators; DOCX body/simple tables; RFC EML and Outlook MSG current body, quoted history, safe headers, inert selected attachments, hash-only thread metadata, and exact RFI/Submittal locators; deterministic PDF structure/table routing; local RapidOCR; bounded text splitting; 300-second PDF progress preservation; 1/2/4-worker ordered PDF page chunks | Mailbox synchronization, automatic attachment recursion, semantic thread inference, borderless/cross-page tables, complete multi-column reading order, complete revision/comment semantics, footnotes, and graphic semantics; process startup can outweigh parallelism on small PDFs |

EML的非空纯文本alternative保持优先；纯文本全部为空白时使用非空安全HTML，因此不会因空壳plain部分丢失RFI回复或Submittal状态。Outlook把`From / Sent / To / Subject`等显式引用头压在同一可见行时仍能开始引用历史，旧问题不会混入当前RFI答复角色。父邮件解析遇到附件即停止向下遍历，附加EML正文及其内部文件不会污染父证据、父附件清单或RFI角色。主工作流按明确Subject、正文标题、文件名排序，较弱来源里的另一类编号保留为精确引用且不能把角色或状态传给主编号；正文证据按明确编号区段显示各自RFI角色或Submittal状态。Subject支持最多6个有界Re/Fw/Fwd及方括号企业安全标签。
...
| 模型 | mock端到端；DeepSeek文本非思考、DeepSeek V4 Flash Vision选择性路由及Gemini文本minimal适配；可识别的Spec/Schedule表格使用一个2,560像素局部裁剪并保留整页坐标，其他合格页面维持一次整页概览；客户可在本机页面配置OpenAI-compatible远端文本API或回环本地模型，端点与模型绑定运行身份；相邻证据最多4段/8.8KB合并为一次串行抽取，相同证据范围的字段核验最多4项合并；指定59页图纸与833页规格书的历史合并运行完成2/2文件、222/222视觉页和全部核验 | 通用入口的供应商专用参数、视觉、自动模型发现和非回环LAN；一般图纸区域检测、局部裁剪live质量/延迟、施工准确率和L2升级；抽取批处理尚无live质量/延迟对比 |
...
工作流索引中的RFI角色只由明确Question/Response形成；UNKNOWN页面上下文保持中性，不会制造完整问答关系。
Submittal正文和精确引用显示为LINKED；邮件自引用标为AMBIGUOUS；只有父文档解析为EMAIL时才显示选定附件来源关系。
Email线程按本地精确哈希父级／引用图让祖先先于回复，同级稳定排序；循环关系保留全部邮件并标AMBIGUOUS，不猜正文语义或日期顺序。同一邮件有多个不同Message-ID时只保存冲突布尔标记并标AMBIGUOUS；没有Message-ID仍作为普通独立邮件显示。
Workflow端点通过SQLite内置JSON投影只读取九个关系字段，不加载页面、视觉、CAD和几何数组；2,000页合成摘要的300次查询/解码基准为2.35×，真实项目收益仍取决于摘要大小和本机SQLite。网页显示已加载/总组数，首批和后续每批最多500组，只有审核者点击时才继续加载并按组ID去重，不再把首批500组静默显示为完整结果。
