...
- Before model dispatch, an exact RFI or Submittal status question checks the complete run-level workflow index; multiple explicit primary statuses return insufficient evidence without a model call, while duplicate sources with one status do not block.
