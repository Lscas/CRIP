...
- Before model dispatch, an exact RFI or Submittal status question checks the complete run-level workflow index; multiple explicit primary statuses return insufficient evidence with each conflicting status, source file and detected-or-manual classification origin rendered directly under the answer. A detected source receives an exact parser-text citation only when the matching bounded status phrase is tied to the exact workflow identity in one statement or a fragment or locator scoped to that identity alone; manual corrections, another workflow's phrase and file-level detections without an exact source phrase remain visibly uncited rather than receiving a fabricated quotation. Duplicate sources with one status do not block.
- An exact unfiltered plural count question for RFIs, Submittals and/or Emails uses the complete terminal-run workflow index before evidence retrieval and returns deterministic RFI/Submittal identifier counts and unique analyzed Email-file counts with an explicit workflow-index basis; filtered, status-qualified, document-source, identifier-specific and content questions do not take this shortcut, and no model call or budget action is added.
