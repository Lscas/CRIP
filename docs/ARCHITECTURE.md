...
RapidOCR/ONNX、PDF选择性整页视觉任务、DXF对象元数据、GNU LibreDWG受限转换和PDF矢量几何审计已接入本机流程。可识别的Spec/Schedule有边框表格可路由到一个保留整页坐标的高分辨率局部裁剪；其他合格页面仍使用一次整页概览。RFC风格EML用Python标准库在本机把当前正文、显式引用历史与头信息分开；HTML blockquote写入成对本机标记，历史块关闭后恢复当前正文状态，显式`From:`加`Sent:`／`Date:`／`To:`／`Subject:`头在同一或相邻可见行时共用一个引用边界，主动内容不执行，远程资源不请求。邮件线程只使用本机SHA-256截断键；Message-ID、References和多值In-Reply-To先逐个拆分再哈希，不保存原标识，也不做语义推断。同一邮件含多个不同Message-ID时不选择赢家，只写入`message_id_conflict`布尔值并让关系组标歧义；缺少Message-ID仍保持独立项。RFI/Submittal正文、Subject和文件名候选都进入同一保守标识规范化器；主上下文按明确Subject、正文标题、文件名的顺序选取，不同工作流编号仍进入精确引用。主上下文只在自己的同类编号区段内读取RFI角色或Submittal状态；逐行分段器遇到另一精确编号时重置局部上下文并把该编号、角色、状态写入对应证据locator。同一主编号重复出现时继续保留既有主状态，不造成兼容性回退。Subject跳过最多6个交错的回复/转发或40字符方括号安全标签，然后仍须明确工作流词和编号。文件名读取完整的有界候选以支持带空格CSI式编号，但只作路由元数据。工作流成员展平状态集合前保留来源内冲突信号，同一多页Submittal的不同显式状态直接进入歧义状态，不自动裁决。附件先保持不活动；父邮件MIME遍历遇到附件边界即停止，不进入附加邮件正文或其后代。审核者可显式选择单个附件，经现有容量、分片、SHA-256和去重路径导入；独立来源表保留父邮件和附件身份，运行Workflow通过一次带索引SQLite查询显示该关系，重复关系合并计数但不继承角色、审批或权威，嵌套附件不会自动递归。确定性守卫按Evidence ID逐条阻止RFI问句、被拒Submittal、邮件头或引用历史作为当前材料/QA直接依据，混合来源仍有有效依据时保留候选；装配阶段检查全部剩余直接来源，使后置的Submittal、RFI答复或Email正文仍产生条件性。只有含数字的RFI/Submittal编号进入关系索引，带空格CSI式编号和常见明确Submittal状态被规范化，无角色RFI仍保持未知。Autodesk/Procore连接器复用已安装`httpx`、现有Uploads路径和Windows DPAPI，不增加SDK或运行时；只列目录和导入选定文件，不执行远程写入。PDF几何只输出页面对象与可能的标定信息，不写入材料设计净量；CAD块/图层计数或已知单位下的长度面积仅输出可追溯、待审核候选。MSG、邮箱同步、自动附件递归、语义邮件线程推断、连接器交互OAuth/刷新/同步、通用图纸区域检测、图纸语义归并、局部裁剪live质量、施工准确率和自动材料净量仍未实现。
