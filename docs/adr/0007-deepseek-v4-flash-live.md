...
配置和离线测试不调用供应商。本机RapidOCR/ONNX、PDF矢量审计及CAD转换不调用供应商；视觉启用时仅发送派生的受限整页PNG、必要定位元数据和提示词，不发送密钥或原始PDF。真实文本/视觉连通、实际账单、整份59页PDF输出稳定性与施工准确率必须在用户本机输入Key后分别验证。整页视觉已接线但局部高分辨率裁剪、施工语义归并和自动材料净量不在本ADR的已交付范围。Gemini历史调用及两条未决费用不因切换供应商被删除或自动结算。
