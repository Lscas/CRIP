...
本文件保留v0.2.1历史结果，不代表当前v0.2.6状态。当前v0.2.6离线测试集（含72项核验及Windows DPAPI密钥保存回归）、DeepSeek V4 Flash文本/视觉真实运行、59页真实PDF文字/OCR重解析与Gemini历史局部运行见`docs/EVIDENCE_VALIDATION.md`。
...
## 验证过的保护
分片重放/哈希不一致、上传超限、空项目、单活跃运行、内部修订日期与上传先后无关、输入快照不可变、未知格式不丢失、人工审核版本冲突、证据非法引用、Excel公式注入防护、DeepSeek非思考、缓存无重复HTTP、未配置不请求、原子预算竞态、超时保留预留、不自动重试、usage缺失、截断/错误JSON/假证据阻止发布、DeepSeek意外思考响应暂停。
