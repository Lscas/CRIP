...
Offline boundary cases accept complete unfiltered count forms for plural RFIs, Submittals and Emails and reject status-filtered, document-count, identifier-specific and content-filtered lookalikes. A synthetic complete workflow index returns two RFI identifiers, one Submittal identifier and two unique Email files, including an `.eml` member whose effective document type was manually changed. The test replaces evidence retrieval with a hard failure and uses a live-configured `httpx.MockTransport`; the answer succeeds with `answer_basis=WORKFLOW_INDEX`, zero retrieved passages, no HTTP request and no model-call ledger row. A route-level fixture separately proves that the project question endpoint loads the complete terminal-run index for this intent. No customer document, credential, live provider or paid API was used.
