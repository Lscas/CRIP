...
## 2026-09-21 project-answer workflow-identifier grounding

Synthetic offline validation starts with a passage identifying `RFI 42` and separately mentioning coordination item `43`; the former numeric-only guard accepted a model answer claiming `RFI 43`, while the new workflow-identity guard rejects it. Additional cases accept `RFI 42` cited to full `Request for Information No. 0042` text or a `RFI 42 > RESPONSE` locator, recognize two exact RFI markers on one line, preserve `RFI ARC-42` versus `RFI ARC-0042`, and reject a `Submittal 23-02` finding cited to `Submittal 23-01`. A locator-grounded `RFI 42` still cannot support an absent 4-inch or 42-inch value cited only to 2 inches, because numeric occurrences outside the identifier remain quote-bound. The full question suite uses synthetic immutable evidence and no customer document, credential, live provider or paid API.

## 2026-09-21 project-answer workflow-number grounding

Synthetic offline validation first reproduces a false acceptance in one exact citation: `RFI 42` requires 2 inches and `RFI 43` requires 4 inches, but the former numeric-only check accepted an answer assigning 4 inches to RFI 42. The new source-local guard rejects that cross-identity transfer while accepting each item's own value. Additional fixtures preserve pure-numeric RFI zero-padding equivalence, accept a number under one unique matching locator identity, reject a multi-identity locator, ignore digits inside the workflow identifier itself, and prevent a Submittal source finding from borrowing another Submittal's number. Existing comparison summaries remain valid because every source finding is checked independently before its values can appear in the summary. The tests use synthetic immutable evidence only and add no provider call, dependency, unit conversion, arithmetic, property-name inference or authority rule.

## 2026-09-21 project-answer measurement-property grounding

An offline reproduction places `Pipe diameter is 2 inch` and `Insulation thickness is 1 inch` under RFI 42; the former workflow-number guard accepted a 1-inch pipe-diameter answer because both values belonged to the same identity. The new bounded role guard rejects that swap while accepting each correct pair and common `2 inch Type L copper pipe` wording. Separate fixtures reject Submittal height reused as width, RFI 43 thickness reused by RFI 42, and Email working pressure reused as compressive strength. Roles are paired only when explicitly labelled or conservatively indicated by a dimensional pipe/duct/tube/conduit or insulation phrase. The tests use synthetic evidence only and add no provider call, dependency, unit conversion, arithmetic, arbitrary property ontology or authority rule.

## 2026-09-22 project-answer measurement-unit grounding

An offline reproduction places `Pipe diameter is 2 inch` and `Insulation thickness is 2 mm` under RFI 42; the former numeric/property checks accepted a 2-mm pipe-diameter answer because the identity, number, property and unit each appeared independently. The new relationship guard rejects that recombination and requires recognized number/unit, property/number/unit and workflow/property/number/unit tuples to remain supported. RFI fixtures accept inch/inches/`in.`/quotation-mark and feet/foot/`ft` formatting equivalents plus compact `2mm`, `150MPa` and `4L/s` forms. Separate Submittal, cross-RFI and Email fixtures reject height-unit-as-width, another workflow's unit, and PSI-as-MPa strength; arbitrary alphanumeric and unsupported rate strings are not reinterpreted as measurements. The checks use synthetic immutable evidence only and add no provider call, dependency, unit conversion, arithmetic or authority rule.

## 2026-09-22 project-answer specification-section grounding

An offline reproduction cites `Specification Section 22 11 16` and `Specification Section 23 05 00` under RFI 42. The former component-number check accepted an invented `Section 22 05 16` because 22, 05 and 16 each appeared somewhere in the citation. The new bounded guard treats an explicitly labelled six-digit CSI/MasterFormat section and optional immediate two-digit extension as one value before generic numeric validation. Synthetic RFI and Email fixtures reject component recombination, a cross-RFI fixture prevents another item from donating its section, and a structured comparison prevents a Submittal finding from borrowing the Specification finding's section. Space, hyphen and dot separators are accepted as formatting equivalents, an exact locator may supply the section context, and an unlabelled Submittal identifier is not parsed as a section. No provider call, dependency, applicability inference or authority rule is added.

## 2026-09-22 project-answer revision-label grounding

An offline reproduction cites `RFI 42 ... Revision A` and `RFI 43 ... Revision B`; the former validator accepted `RFI 42 ... Revision B` because alphabetic revision labels were outside numeric, date, section and workflow-identifier checks. The new bounded guard keeps an explicitly written Revision/Rev label atomic and attached to the same RFI/Submittal statement. Synthetic RFI and Email fixtures reject cross-workflow borrowing, an A1/B2 fixture rejects A2 recombination, and a structured comparison prevents a Submittal finding from borrowing a Specification finding's revision. Prefix punctuation/case variants and bounded IFC/IFB/AS BUILT forms normalize; numeric labels are excluded from quantity checks. Revision dates, unlabelled status words and metadata-only labels remain outside this claim path. No provider call, dependency, chronology or authority inference is added.

## 2026-09-22 project-answer drawing-identifier grounding

An offline reproduction cites `Sheet A1.01` and `Sheet P2.02`; the former validator accepted invented `Sheet A1.02` because the alphanumeric identifier was outside the existing atomic-value guards and its remaining numeric component appeared elsewhere. The new bounded guard keeps explicit Sheet/Drawing/Dwg identifiers atomic and attached to the same RFI/Submittal statement. Synthetic RFI and Email fixtures reject cross-workflow borrowing, a structured comparison prevents a Submittal finding from borrowing the Specification finding's sheet, and a locator fixture allows only its exact `Sheet` value. Prefix variants, letter case and dash glyphs normalize; numeric sheet IDs are excluded from quantity checks. Unlabelled codes and date-shaped values remain outside this claim path. No provider call, dependency, applicability, revision or authority inference is added.

## 2026-09-22 project-answer detail-callout grounding

An offline reproduction cites `Detail 3/A5.1` and `Detail 5/A7.2`; the former drawing guard did not recognize Detail prefixes, so numeric-component validation accepted invented `Detail 3/A7.2`. Detail now routes through the same bounded drawing-identifier guard. Synthetic RFI evidence rejects component recombination, an Email fixture rejects a Submittal detail borrowed by RFI 42, and Detail/Detail No./Detail: prefix forms normalize while the slash-form value remains exact. No provider call, dependency, drawing-geometry interpretation, applicability or authority inference is added.

## 2026-09-22 project-answer clause-identifier grounding

An offline reproduction cites `Paragraph 2.3.1` and `Paragraph 4.5.6`; the former numeric check accepted invented `Paragraph 2.3.6` because its decimal and final numeric components appeared separately. The new bounded guard keeps explicit dotted Paragraph/Para, Clause and Article identifiers atomic and typed. Synthetic RFI and Email fixtures reject cross-workflow borrowing, a structured comparison prevents a Submittal finding from borrowing the Specification finding's paragraph, and a locator fixture allows only its exact Paragraph value. Paragraph/Para plus punctuation/case variants normalize, while Paragraph, Clause and Article remain distinct. Unlabelled, undotted and date-shaped values remain outside this claim path. No provider call, dependency, hierarchy, applicability or authority inference is added.

## 2026-09-21 project-answer date grounding

Synthetic offline validation first reproduces the former false acceptance: a citation states `2024-01-05` and `2024-02-10`, while the answer recombines them as `2024-01-10`. The new source-local full-date guard rejects that ISO case and its English month-name equivalent. Formatting-only ISO leading zeros, full/abbreviated English month names, month-first/day-first English forms and slash-date leading zeros remain valid. A slash date does not support an ISO rendering because its locale is ambiguous. A comparison fixture also proves that a date in the Specification finding cannot support a different RFI finding. The tests use synthetic immutable evidence and no customer document, credential, live provider or paid API; they do not validate calendar legality, calculate dates or infer chronology or authority.

Additional RFI, Submittal and Email fixtures pair issued/due, submitted/approved and sent/received dates inside the same exact citation. Wrong pairings fail even though the claimed date appears elsewhere in that quotation; equivalent ISO and English month-name forms pass. A label-only citation plus a separate date-only citation cannot manufacture a pair, and a comparison finding cannot borrow another finding's date role. Two approval dates in one cited scope remain ambiguous. A date question can report the one explicit approved date from a submitted/approved lifecycle record, while the same record still fails an ordinary status question as conflicting. These deterministic checks add no request and make no chronology or current-state inference.

Workflow-scope fixtures then place separate dates for RFI 42/RFI 43 and Submittal 23-01/23-02 inside one exact quotation. The validator rejects a date role moved between identities, accepts each item's own independently labeled date, preserves `RFI 42`/`Request for Information No. 0042` equivalence and accepts an identity omitted from text only through a locator containing that one exact identity. A locator containing two identities stays unscoped. Multiple dates for the same identity and role remain ambiguous. The tests use synthetic evidence only and add no provider call, date ordering or authority inference.
