...
## 2. 范围
首版输入PDF、DOCX、TXT、RFC风格EML、PNG/JPEG/TIFF/BMP/WEBP等常见图片、DWG。EML仅在本机解析安全头信息和可见正文，附件登记为未处理并要求单独上传；不执行HTML、不加载远程资源。非支持图像或格式保留清单并说明，不以“所有图片”宣称无限格式支持。Excel、MSG、邮箱连接／同步、BIM原生和持续同步不自动加入本轮范围。
...
DOCX保留标题、列表、表格、嵌入图片以及稳定段落定位。可解析的修订和批注要区分正文，不确定采用状态则标记。TXT保留编码与行号。EML把Subject/From/To/Cc/Date与正文分开定位；优先纯文本正文，只有HTML时仅保留安全可见文字。明确RFI问句／答复和Submittal状态进入locator；RFI问句不得成为材料、测试或修订指令，Submittal/Email来源候选保持条件性与人工审核。完整产品中的图片保留原尺寸、旋转、裁剪变换和OCR文字；当前已提供本机OCR和页面级视觉任务，但未实现通用局部裁剪变换。DWG用合法可配置转换服务，缺Xref、字体、自定义对象、单位或Layout明确报出；当前转换和对象解析均成功时才标记`OBJECT_METADATA`，转换器缺失、转换失败或对象不可解析均标记`UNAVAILABLE`；不将部分对象解析冒充为完整CAD语义。
