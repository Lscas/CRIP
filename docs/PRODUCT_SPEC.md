...
工作流读取只从持久化解析摘要投影关系所需字段，不把页面、视觉、CAD或几何数组加载到Python；这不改变关系语义或分页结果。
