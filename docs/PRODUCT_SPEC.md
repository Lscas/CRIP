...
Email routing headers and quoted history remain stored and reviewable for exact relationships, but they are deterministically completed as no-current-requirement before extraction batching. Only eligible current evidence reaches the model; Coverage reports how many fragments were skipped locally.
...
工作流读取只从持久化解析摘要投影关系所需字段，不把页面、视觉、CAD或几何数组加载到Python；这不改变关系语义或分页结果。浏览器必须显示已加载数和实际总数，只在审核者要求时继续加载下一批最多500组，并按关系组ID去重追加，不能把首批500组冒充为完整结果。
