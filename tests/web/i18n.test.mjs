...
const catalog=JSON.parse(source.match(/const catalog = ([\s\S]*?);\r?\n  const statusCatalog/)[1]);
const statusCatalog=JSON.parse(source.match(/const statusCatalog = ([\s\S]*?);\r?\n  const systemEnglish/)[1]);
const nodes=[];
...
test('default language is English',()=>{const x=setup();assert.equal(x.I.language,'en');assert.equal(x.label.textContent,'＋ New project');});
...
test('unsupported persisted value falls back to English',()=>assert.equal(setup('bad').I.language,'en'));
...
test('unsupported selection does not write or change display',()=>{const x=setup();assert.equal(x.I.setLanguage('de'),false);assert.equal(x.writes.length,0);assert.equal(x.I.language,'en');});
...
test('parameters retain exact numeric values, currency, scale and selected limit',()=>{const x=setup('en');assert.equal(x.I.t('run.cost',{spent:'12.3400',reserved:'1.0000',limit:'750.00'}),'Recorded API cost ¥12.3400 · Reserved ¥1.0000 / Limit ¥750.00');});
...
test('raw document text is never auto-translated',()=>{const x=setup('en');const n=node();x.I.bindText(n,()=> '项目名：混凝土 / Concrete');assert.equal(x.I.setLanguage('zh-CN'),false);assert.equal(n.textContent,'项目名：混凝土 / Concrete');});
...
test('detached bindings are safely discarded',()=>{const x=setup();const n=node();x.I.bindText(n,'run.start');n.isConnected=false;x.I.setLanguage('en');assert.equal(n.textContent,'Start analysis');});
test('known legacy system notices display in English',()=>{const x=setup();const n=node();x.I.bindMessage(n,'审核记录已保存');assert.equal(n.textContent,'Review saved');assert.equal(x.I.setLanguage('zh-CN'),false);assert.equal(n.textContent,'Review saved');});
...
test('budget guard errors are translated',()=>{const x=setup('en');assert.equal(x.I.message('项目预算不能低于已支出和未结算预留合计'),'Project budget cannot be lower than settled cost plus outstanding reservations');});
